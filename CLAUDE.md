...
- **Name joins** across Whiteboard / B-T tab / Lookup use `normName_` (lowercase, trim, collapse spaces) and rely on "First Surname" spelling agreement; mismatches surface as `skipped` (lunch) or unmatched cards (staff pick) — never silently dropped.
